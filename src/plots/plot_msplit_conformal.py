...
def plot_multidim_msplit(split, same_scale=False, color="blue", ci_color="red", figsize=(10, 8)):
    ...
    Parameters:
        ...
        color (str): Color for the scatter points. Default is "blue".
    ...
    if p == 2:
        ...
        for jj in range(q):
            ...
            # Scatter plot for independent variables (x1, x2)
            ax.scatter(x1, x2, y_min, color=color, label="Data Points")
            ax.scatter(x1, x2, y_max, color=color)
        ...
        fig.suptitle("Confidence Regions without Predictions", fontsize=16)
        ...
        for ii in range(p):
            for jj in range(q):
                ...
                sns.scatterplot(
                    data=df, x='xd', y='y_min', ax=ax,
                    label="Lower Bound", color=color, s=20
                )
                sns.scatterplot(
                    data=df, x='xd', y='y_max', ax=ax,
                    label="Upper Bound", color=color, s=20
                )
                ...
                if same_scale:
                    ...
                    y_min, y_max = df[['y_min']].min().min(), df[['y_max']].max().max()
